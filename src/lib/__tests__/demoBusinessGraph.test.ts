...
describe('complete deterministic Demo business graph',()=>{
 ...
 it('has unique IDs and exact tenant ownership',()=>{const p=plan();const keys=p.documents.map(d=>`${d.collection}/${d.id}`);expect(new Set(keys).size).toBe(keys.length);expect(p.documents.filter(d=>d.collection!=='user_auth_maps').every(d=>d.data.companyId===DEMO_COMPANY_ID&&d.data.demoSeedId===DEMO_SEED_ID)).toBe(true)});
