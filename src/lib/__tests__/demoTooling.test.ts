...
describe('trusted Demo Mode tooling',()=>{
 ...
 it('builds an idempotent unique foundation plan',()=>{const a=buildFoundationPlan('auth-uid');const b=buildFoundationPlan('auth-uid');expect(a).toEqual(b);expect(a.documents).toHaveLength(91);expect(new Set(a.documents.map(d=>`${d.collection}/${d.id}`)).size).toBe(91);expect(verifyPlan(a)).toEqual([]);expect(()=>assertNoForbiddenFields(a.documents)).not.toThrow()});
