import { batchCreate, createDocWithId, deleteDocById, getOne, resolveWriteCompanyId, updateDocById } from './firestore';
...
function hydrateCreatePayload<T extends Record<string, unknown>>(payload: T): T & { companyId: string; createdBy: string; updatedBy: string } {
...
export async function updateProjectionWithEntity<T extends Record<string, unknown>>(
  ...
  const hydrated = hydrateUpdatePayload(payload);
