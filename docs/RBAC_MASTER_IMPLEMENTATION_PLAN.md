...
### PHASE 10 — FINAL VERIFICATION PASS (2026-09-09, post BD-3/BD-5)

A dedicated final verification of the Phase 10 close, on top of `f5ad9ab` (BD-3 + BD-5 committed). Two small code corrections + a targeted performance pass; **no business-logic or security-policy change**.

**TASK 1 — BD-3 final safety verification (behaviour re-confirmed, no policy change):**
- verified/active partner (incl. KYC pending / not_started / submitted / rejected) → login OK, create Lead/Customer/Project/Scheme OK, commission generated. ✔ (`partnerLifecycleEligibility.emulator.test.ts`, `partnerEligibility.test.ts`)
- suspended → login OK, existing/in-flight work readable + updatable, **NEW** Lead/Customer/Project/Scheme BLOCKED at all four planes (rules `partnerCreateEligible()`/`channelPartnerStatusActive()`; REST `assertApiPartnerCanCreate` → 403; workflow `assertPartnerCanCreate`; client modals). Pre-suspension earned commission untouched — `generateCommissionRecord` only skips `status === 'inactive'`, never `suspended`. ✔
- inactive/terminated → linked login deactivated (`transitionStatus` → `users.status='Inactive'` → `actorIsActive()` denies everything + `onUserDeactivated` revokes tokens); create + commission blocked independently. ✔
- **TASK 1.6 defect found + fixed:** `transitionStatus` reactivation could re-enable a login that was independently deactivated (HR/security hold) when a partner flips back from `inactive`. Fixed with a `partnerLifecycleHold` marker on the users doc — reactivation restores `status:'Active'` **only** when this mechanism set the hold. Both login writes are now wrapped best-effort (a non-Admin actor can't write the users doc; the partner-status write already succeeded and still gates creation). `src/services/ChannelPartnerDomainService.ts`.
- suspension cannot be bypassed via REST or client manipulation — the `channelPartnerId` link is server-resolved (`api/_lib/auth.ts`, `resolveCurrentPartnerDocId`), never trusted from the payload; rules re-derive it from `currentUser()`.

**TASK 2 — scheme-registration suspended-creation gap resolved:**
- The `scheme_registrations` rules block remains not directly write-testable on the emulator (create+update+generic-fallback coverage summation > 1000 expressions — pre-existing §10.4 / AUTH-S1b, true even for Admin and even pre-BD-3; no scheme emulator write test has ever existed).
- Resolved with a **real runtime regression test** — `channelPartnerPhase6SchemeRegistration.test.ts › BD-3 — partner lifecycle gate on scheme creation` (6 cases): ACTIVE partner CAN file; no-status partner CAN file (grandfathered); SUSPENDED / INACTIVE / pending_approval partner CANNOT file (`createSchemeRegistration()` actually executes and calls `assertPartnerCanCreate` on the server-resolved `channel_partners` record); a non-partner staff actor is not gated. **52/52 pass.**
- Emulator-proven twin: `channelPartnerStatusActive()` — the exact helper `schemeRegPartnerOwnsProject()` calls — is verified against the real emulator for leads/customers/projects in `partnerLifecycleEligibility.emulator.test.ts` (active CAN, suspended/inactive CANNOT).

**TASK 3 — fresh full Firestore rules gate (`npm run test:rules`, current Phase 10 HEAD, `firestore.rules` = `861643a` byte-for-byte):**
- **Single complete local pass:** 33 files / 840 tests → **825 passed / 15 failed, exit 1, ~9 min.** All 15 failures are the documented §2.1 cold-run flake — `FirebaseError: [code=unavailable] … client is offline` / "Backend didn't respond within 10 seconds" / a query returning `+0` rows under connection saturation — spread one-or-two per file across 9 files, never a `PERMISSION_DENIED`/allow-deny assertion mismatch.
- **Flake control:** the 9 affected files re-run in two batches → **batch A 5 files / 111 tests PASS; batch B 4 files / 443 tests PASS** (554 tests, 100% green, zero rule failures). Every one of the 33 emulator suites has now passed against the current `firestore.rules` — 24 in the clean single-pass, 9 in the targeted re-runs.
- **CI single-pass NOT run:** `.github/workflows/security-rules-tests.yml` triggers only on `push`/`pull_request` to `main` (no `workflow_dispatch`); this task forbids any push to GitHub. The mandated one-clean-CI-run gate is therefore **outstanding** — it needs either an explicit owner authorisation to push a throwaway `ci/*` branch (non-merged draft PR, Phase 9 procedure) or acceptance of the local single-pass + batched-green evidence above.

**TASK 4 / 5 — performance pass (source-level; see the Phase 10 final report §9 for the full audit):** 4 safe optimisations, each verified no functional/security regression —
1. `resolveCurrentPartnerDocId` fast path (`src/lib/partnerOwnership.ts` + session carries `users.channelPartnerId` via `userProfile.ts`/`Login.tsx`/`useAppStore.ts`): removes **2 sequential Firestore round-trips per `getAll()`** (333 call sites) for every non-Partner session — the old code only cached a *successful* resolution.
2. `ProtectedRoute` selector (`useAppStore(s => s.isAuthenticated)` not full-store): the wrapper above every authed route no longer re-renders its whole subtree on every `setRoleData` / `setPermissionCache` / company switch.
3. `useDashboardData` staleTime 45s → 3min (+ gcTime 10min): the ~30-query dashboard fan-out is no longer re-run on every in-session return to the dashboard; still tighter than the 5min app default.
4. `useGlobalBoot` merges the boot `getAll(USERS)` onto the shared `['users']` query key (was a private `['users_hierarchy']` key used nowhere else) — one warm read instead of two full `users` reads for Manager/TL sessions; removed a duplicated early-return.
- **Not changed** (per the 12 rules): the 2.75 MB `index` bundle (needs analyzer + route code-split — high risk), the dashboard defensive double date-range query (removing a path risks missing rows), the other 44 no-selector `useAppStore()` call sites (page-local, low blast radius), any Firestore rule.

**TASK 6 — regression gate (current HEAD + working tree):**
- `npx tsc --noEmit` → exit 0.
- `npm run build` → exit 0 (`built in 2m 3s`; only the pre-existing chunk-size notice; `index` 2,745.83 kB / gzip 635.29 kB — unchanged).
- `npx vitest run --config vitest.api.config.ts` → **404 / 404 pass** (18 files).
- `npx vitest run` (full) → **29 failed files / 65 failed tests / 3819 passed (280 files, 3884 tests)** — the Phase 9 baseline **29 / 65 unchanged**; +46 passed / +46 total vs. baseline are new BD-3/perf tests; **zero new failures**. The 29 failed files are exactly the known brittle source-text UI-parity suite (§35).
- `npm run test:rules` → single-pass 825/840 (15 environmental flakes) + affected-file re-runs 100% green (above).

**Git:** working-tree changes local only (11 files). `firestore.rules` reverted to `861643a` — the two experimental helper micro-trims (`partnerLinkedToSelf`, `schemeRegManagerOfPartner`) from an earlier draft were dropped; **no rules change ships in this pass.** `main` = `origin/main` = `0d9699f`, untouched. Nothing pushed / merged / deployed. **There is no Phase 11.**
