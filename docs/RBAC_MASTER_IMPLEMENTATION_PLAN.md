...
| AUTH-D10 | (new) | Medium | `api/_lib/biometrics/authorization.ts` gates biometric enrollment on a raw `auth.role !== 'Admin' && auth.role !== 'HR' && !auth.isSuperAdmin` check — the same alias-blind, GroupAdmin-false-DENY pattern AUTH-D5 named for `/api/integrations`, at a different endpoint the Master Plan never enumerated | **Discovered — Phase 6, deliberately not fixed** (not named in this phase's scope; a mid-phase discovery does not authorize expanding the phase). Candidate for a future phase or an explicit instruction. |
...
- **Production / `main` impact:** NONE. `main` = `origin/main` = `0d9699f`, unchanged and verified. N1/N2 are local commits on `local/rbac-phase1-8-and-mobile` only (`7f90239`, `c665794`). No push, no merge, no PR, no deploy; the deployed Firestore ruleset is unchanged.
...
| Phase 10 | **IN PROGRESS — BLOCKED.** Fresh independent audit run; §12 otherwise re-verified closed and §7/§9/§10 baseline re-confirmed (zero unexplained deltas). Two new findings surfaced and remediated in code: **N1** (AUTH-C1 REST-API plane — `/api/leads` + `/api/customers` now enforce self/team ownership, READ-only, no rules change) `c665794`; **N2** (AUTH-D9 — mobile Installations edit gate now `perms.canEdit('installations')`) `7f90239`. Combined verification: `npx tsc --noEmit` exit 0; `npm run build` exit 0; `npx vitest run --config vitest.api.config.ts` 388 pass; `npx vitest run` = documented baseline unchanged (29 failed files / 65 failed tests, +4 passing N2 tests). `npm run test:rules` not required (`firestore.rules` byte-identical; Phase 9 CI pass `34216974918` stands). **NOT complete, NOT "RBAC READY":** blocked on BD-3..BD-8 (§15, unresolved) and CP-1 (blocked by BD-3, deliberately untouched). Local commits only, not pushed; `origin/main` untouched at `0d9699f`; no deploy. Interim record in §18. | `7f90239`, `c665794` | 2026-09-08 |
