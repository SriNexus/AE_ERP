...
| Phase 10 | **BLOCKED — awaiting owner decisions (final determination; three independent BD-3 passes).** Fresh independent audits run; every previously-CLOSED `AUTH-*` re-verified closed against current source; §7/§9/§10 baseline re-confirmed (zero unexplained deltas). Two enforcement-plane divergences surfaced and **remediated in code**: **N1** (AUTH-C1 REST-API plane — `/api/leads` + `/api/customers` now enforce the same self/team ownership as `firestore.rules`, READ-only, Option B, no rules change) `c665794`; **N2** (AUTH-D9 — mobile Installations edit gate now `perms.canEdit('installations')`, desktop parity) `7f90239`. Doc hygiene: §12 AUTH-D10 → CLOSED (`38f3077`), AUTH-C2 → SUBSUMED/RESOLVED. Verification: `npx tsc --noEmit` exit 0; `npm run build` exit 0; `npx vitest run --config vitest.api.config.ts` 388/388; `npx vitest run` = documented baseline unchanged (29 failed files / 65 failed tests / 3777 passed, +4 passing N2 tests). `npm run test:rules` not re-run (would be incomplete on the sandbox — not claimed); `firestore.rules` byte-identical to Phase 9 close, so the Phase 9 CI pass `34216974918` (32/32 files, 818/818) remains authoritative. **NOT "RBAC READY", NOT "Phase 10 COMPLETE".** Two irreducible owner blockers: **BD-3 / CP-1 / CP-2** (exact suspend/KYC action matrix — cannot be resolved from authoritative source without inventing policy or regressing legitimate KYC-pending onboarding; `validatePartnerCanAct` blocks all non-`verified` KYC and would lock out every state-C partner; "generate commission for a pre-suspension deal" is genuine earned-money business policy) and **BD-5** (`Management` alias — any repoint alters effective access for any live `Management` account; non-existence not provable from source; AUTH-S2 "don't touch a mapping without evidence it's dead" precedent). BD-4 / BD-6 / BD-7 / BD-8 carry no security dimension and their current state already equals the recommended safe default — open only for §22 owner sign-off, not code. Local commits only (`7f90239`, `c665794`, docs); `origin/main` untouched at `0d9699f`; nothing pushed/merged/deployed. Interim + final-determination records in §18. | `7f90239`, `c665794` | 2026-09-08 |
