...
```
CURRENT PHASE:          INVENTORY-08 — Warehouse Transfer — IMPLEMENTED + VERIFIED + COMMITTED
                        (a SEPARATE commit after 06+07). Phases 06+07 committed together as
                        `de5902e`; Phase 08 is its own commit on top.
...
HEAD     refactor(inventory): single writer — retire duplicate stock transaction (INVENTORY-05d, P1-4)   (current HEAD — also carries this STATE + regression-matrix update; a later session backfills the real short hash)
...
TOGETHER as `de5902e` (reconciliation + sales reservation). INVENTORY-08 (warehouse
transfer) implemented + verified + committed as its OWN commit on top. `reservationsEnabled`
is ON by default. The next authorized phase is INVENTORY-09 (master-data integrity) —
it requires its own explicit go-ahead; do NOT start it from this file alone. The Phase-07
backfill (`scripts/inventory/backfill-onhand.ts --apply`) has NOT been run against live
data — do that against a copy, reviewed, before relying on `onHandQty` for pre-05 summaries.
```
