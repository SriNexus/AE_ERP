...
| **08 STATUS (2026-09-04 — SEPARATE commit, after 06+07 `de5902e`)** | **PASS.** First-class warehouse-to-warehouse transfer. New collection `stock_transfers/{TRF-*}` — `{ companyId, groupId, fromWarehouseId, toWarehouseId, warehouseIds:[from,to], items[{productId,qty,unit,shippedQty?,receivedQty?}], status: draft\|in_transit\|received\|cancelled, hasShortfall?, shortfallQty?, shippedBy/At, receivedBy/At, cancelledBy/At/Reason }`. New `src/features/warehouses/services/warehouseTransferWorkflow.ts`: **createTransfer** (draft — validates same-company, both warehouses real, products real, qty>0, from≠to; NO stock effect), **shipTransfer** (`applyStockMovements(TRANSFER_OUT[] @ from, transferParticipant)` → status `in_transit` + shippedQty; ATOMIC — one short line commits nothing — L1/#20), **receiveTransfer** (`applyStockMovements(TRANSFER_IN[] @ to for the qty that ARRIVED, transferParticipant)` → status `received` + per-line receivedQty + `hasShortfall`/`shortfallQty`; partial receipt supported — L2/§7/#32–37), **cancelTransfer** (`draft` → doc flip only; `in_transit` → compensating `TRANSFER_IN` back to source, status `cancelled` — L5/§8). **Engine (`stockMovementEngine.ts`): ZERO change needed** — `TRANSFER_OUT`/`TRANSFER_IN` were already in `MOVEMENT_DIRECTION` (OUT/IN); INV-1 guards the source, Phase-07 INV-3 (final per-summary state) blocks a transfer that would drop onHand below reservedQty (#45–46). Deterministic idempotency keys `TRANSFER_OUT:transfer:{TRF}:{pid}` / `TRANSFER_IN:transfer:{TRF}:{pid}` / `TRANSFER_IN:transfer_cancel:{TRF}:{pid}` → ship/receive/cancel retry-safe (L4/#21–26). **INV-11:** a completed transfer's `Σ(TRANSFER_OUT + TRANSFER_IN) == 0` — machine-verified against the actual `stock`/`stock_ledger` (unit + emulator L3); a lossy transfer's `-shortfall` is an identified, explained discrepancy on the doc (never silently erased). **Reconciliation (additive, Phase-06/07 math untouched):** `computeTransferReconciliation` (PURE) + `reconcileTransfers()` — an `in_transit` transfer is an EXPECTED outstanding movement, NOT unexplained drift; `received` with unbalanced pair → `loss_in_transit` surfaced for a human RECONCILE_ADJUST at the source. Per-summary reconciliation is naturally correct during in-transit (the TRANSFER_OUT row IS in the source ledger). **firestore.rules:** new `warehouseIdInCompany(whId, companyId)` helper; new `match /stock_transfers` block — company+group scoped read (`canReadCompanyScoped`), LEAN `warehouseActorCanCreate/Update` + `actorRoleMatches('.*Warehouse.*\|.*Operations.*\|.*Procurement.*\|Admin\|GroupAdmin')`, BOTH warehouses must exist AND belong to the doc's company (`warehouseIdInCompany` — cross-company impossible: L6/#38–39/§4), warehouse-restricted actor may only create/act on transfers touching their own warehouse, transfer identity immutable, `delete: if false`; added to `isSpecialCollection()`. The physical stock writes still pass through the existing `match /stock` guard (a Sales-role ship is DENIED at the `stock` layer — §13). `COLLECTION_PERMISSION_MODULE[STOCK_TRANSFERS]='stock'`. **firestore.indexes.json:** 3 additive composites (`companyId+isDeleted+createdAt`, `companyId+isDeleted+status`, `groupId+isDeleted`). **UI:** new `/stock-transfers` desktop page (`WarehouseTransfersWorkspace.tsx` + `useStockTransfers.ts` hook) — list/filter, create modal, detail modal with ship/receive (per-line received qty)/cancel, shortfall banner; nav entry under Inventory (`canDo('view'\|'create'\|'edit','stock')`). Mobile: none added (desktop is the source of truth; a mobile shell would call the same hook). **Operational note:** a transfer needs an inventory operator (Warehouse@source ships, Warehouse@dest receives) OR a warehouse-any operator (Admin/GroupAdmin/Procurement) for both legs — a source-warehouse user cannot create the destination stock summary (`match /stock` `sameWarehouse` — the existing Phase-3 isolation). **Tests:** `warehouseTransferWorkflow.test.ts` 18/18 (lifecycle 1–12, validation 13–20, idempotency 21–26, cancel 27–31, partial receipt 32–37, reservation compat 45–46); recon engine +5 (INV-11 balanced / in-transit-not-drift / loss / cancel-return / `reconcileTransfers` aggregate). **Emulator** `stockTransferTransaction.emulator.test.ts` 13/13 — L1–L5, INV-11, double-ship + double-receive idempotent, **concurrent ship (one effect)**, **concurrent ship racing last units (no negative stock, Firestore serialises)**, partial receipt (source −10 / dest +8 / loss visible), Admin both-legs, Sales DENIED, warehouse-restricted cannot ship another warehouse, cross-company create DENIED (both directions), cross-company read DENIED, transfer identity immutable, delete DENIED, ledger immutable, suspended group DENIED. Standard gate: **tsc exit 0 / lint 3 pre-existing attendance / build exit 0 / full vitest 29-fail brittle baseline UNCHANGED / emulator (batched) 100%** — inventory batch 9 files/102 + security batch. Manual verification = the emulator L1/L2/L3 lifecycle test (real Firestore, real transactions, actual persisted-state assertions — the exact §24 checklist). Committed as a SEPARATE commit after 06+07. |
